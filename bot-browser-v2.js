...
const GAS_API = 'https://script.google.com/macros/s/AKfycbzH5cJZUrAZNkabX6XmwwgVpwZcX2KJVMYEdskdSCHQLtjFu7sCmeNN1FxDuxgdNzWe/exec';
const REDIRECT_URI = 'https://donson85001.github.io/punnimantobot/';
const SCOPES = ['user:read:chat','user:write:chat'];
const BUILD = 'browser-v2-20260826-legacy-gas-url';
...
function canAddSong(login){return ADD_SONG_USERS.has(norm(login))}
function log(...args){
...
function isHtml(text){const s=norm(text);return s.startsWith('<!doctype')||s.startsWith('<html')}
...
function logout(){
  stop();token='';me=null;broadcaster=null;
  sessionStorage.removeItem('twitch_access_token');
  loginStatus.textContent='尚未登入';loginStatus.className='status';startBtn.disabled=true;
  log('已清除登入');
}
...
async function handleChat(event){
  ...
  const userLogin=norm(event.chatter_user_login||event.chatter_user_name||'chat');
  ...
  if(text==='!bot健康'||text==='!bothealth'){
    await say(`@${user} bot在線，Twitch監聽正常`);
    return;
  }

  if(text==='!新增點歌'){
    if(!canAddSong(userLogin)){await say(`@${user} 你沒有權限使用 !新增點歌`);return;}
    await say(`@${user} 請輸入歌名，例如：!新增點歌 青花瓷`);
    return;
  }

  if(text.startsWith('!新增點歌 ')){
    if(!canAddSong(userLogin)){await say(`@${user} 你沒有權限使用 !新增點歌`);return;}
    const q=clean(text.slice('!新增點歌 '.length));if(!q)return;
    try{
      const r=await callGAS('chat_add',{user:userLogin,q,channel:room,room});
      await say(r);
    }catch(e){
      log('ADD_SONG_ERROR',e.message);
      await say(`@${user} 新增歌曲失敗：${clean(e.message)}`);
    }
    return;
  }
  ...
  if(text.startsWith('!點歌 ')){
    ...
    try{
      let r=await callGAS('chat_suggest',{user,q,channel:room,room});
      if(r.includes('找不到「') && !r.includes('!新增點歌')){
        r += `，請噗妮妮輸入：!新增點歌 ${q}`;
      }
      await say(r);
    }catch(e){
...
async function subscribe(sessionId){
  ...
  log('EventSub 訂閱成功',d.data?.[0]?.status||'enabled');
  setBotStatus(`監聽中：${broadcaster.display_name}`,true);
}
function connect(url='wss://eventsub.wss.twitch.tv/ws?keepalive_timeout_seconds=30'){
  ...
  socket.onmessage=async e=>{
    try{
      ...
      if(type==='session_welcome')await subscribe(m.payload.session.id);
        ...
        const u=m.payload?.session?.reconnect_url;
        if(u){log('Twitch 要求重新連線');const old=socket;socket=null;connect(u);setTimeout(()=>{try{old.close()}catch{}},1000)}
      }else if(type==='revocation'){
        log('訂閱被撤銷',m.payload?.subscription?.status);setBotStatus('訂閱被 Twitch 撤銷');
      }
  ...
  socket.onclose=()=>{
    log('WebSocket 已斷線');
    if(!intentionalStop){setBotStatus('斷線，正在重連…');clearTimeout(reconnectTimer);reconnectTimer=setTimeout(()=>connect(),3000)}
  };
}
...
function stop(){
  intentionalStop=true;clearTimeout(reconnectTimer);if(socket){try{socket.close()}catch{}socket=null}
  stopBtn.disabled=true;startBtn.disabled=!token;channelInput.disabled=false;setBotStatus('已停止');
}
